...
for i=14:27
    input_szz_filename = "settings/27site/szz_" * string(i) * "_nn_list.csv"
    input_sz_filename = "settings/27site/sz_"* string(i) *"_upstate.csv"
    ...
    input_sxx_filename = "settings/27site/sxx_" * string(i) * "_nn_list.csv"
    input_sz_filename = "settings/27site/sz_"* string(i) *"_upstate.csv"
    marker_magnification = 20
    line_magnification = 40
    ...
    MHdata_file="settings/27site/MHdata.csv"
    MHdata_output="img/MHcurve.png"
    plt_MHcurve = kagome_spinrel.plot_27site_MH_curve_with_Highrigt(MHdata_file, i)

    plt_lattice = kagome_spinrel.plot_27site_kagome_lattice(0.87, 0.06, 1.0)
...
gif(anim, "27site_MHcurve_Ykapellasite.gif",fps=1)
