...
#===========================================処理内容===========================================================
# - ./output/に出力された各MHdata_xxx.csvの3行目と6行目からJ_redの値とplateau widthの値を
#   抽出したファイルとしてplateau_width_dif_Jr_Jg.csvがある
# - 本スクリプトはJg00~Jg10のplateau_width_dif_JgXX.csvのデータをmulti plotするためのものである
# - 1つのグラフに1/3、5/9、7/9プラトーのplateau幅の変化をまとめ、それを2行3列でまとめる
# - outputファイルは2枚用意することにし、1枚目にJ_g=0.0 ~ J_g=0.5、2枚目にJ_g=0.6 ~ J_g=1.0のグラフをplotする
#==============================================================================================================
...
#gnuplotスクリプトファイルを作成
    ...
    set title "J_g=0.0"
    plot for [file in "${files00[@]}"] file using 1:2 with points notitle

    set title font "Arial,10"
    set title "J_g=0.1"
    plot for [file in "${files01[@]}"] file using 1:2 with points notitle

    itr=-1
    set title font "Arial,10"
    set title "J_g=0.2"
    plot for [file in "${files02[@]}"] file using 1:2 with points title sprintf("M=%d/9",itr=itr+2)
    
    set title "J_g=0.3"
    plot for [file in "${files03[@]}"] file using 1:2 with points notitle

    set title "J_g=0.4"
    plot for [file in "${files04[@]}"] file using 1:2 with points notitle

    itr=-1
    set title "J_g=0.5"
    plot for [file in "${files05[@]}"] file using 1:2 with points title sprintf("M=%d/9",itr=itr+2)
    ...
    set title "J_g=0.6"
    plot for [file in "${files06[@]}"] file using 1:2 with points notitle

    set title "J_g=0.7"
    plot for [file in "${files07[@]}"] file using 1:2 with points notitle

    itr=-1
    set title "J_g=0.8"
    plot for [file in "${files08[@]}"] file using 1:2 with points title sprintf("M=%d/9",itr=itr+2)
    
    set title "J_g=0.9"
    plot for [file in "${files09[@]}"] file using 1:2 with points notitle

    itr=-1
    set title "J_g=1.0"
    plot for [file in "${files10[@]}"] file using 1:2 with points title sprintf("M=%d/9",itr=itr+2)
