...
# - 1つのグラフに1/3、5/9、7/9プラトーのplateau幅の変化をまとめる
#=================================================================================================
...
if [ -n "$input_file1" ] && [ -n "$input_file2" ] && [ -n "$input_file3" ]; then
    ...
    #gnuplotスクリプトファイルを作成
        ...
        replot "${input_file2}" using 1:2 pointtype 8 title "M = 5/9"
        replot "${input_file3}" using 1:2 pointtype 6 title "M = 7/9"
        
        set terminal postscript eps color enhanced "Arial" 20
        set output "${output_filename}.eps";replot
        ...
        set output "${output_filename}.png";replot
